...
const DocumentUpload: React.FC = () => {
	...
	useEffect(() => {
		// Check for authentication data using consistent accessToken key
		const storedDriverId = localStorage.getItem('driver_id');
		const accessToken = localStorage.getItem('accessToken');
		
		console.log("Checking authentication...");
		console.log("Driver ID:", storedDriverId);
	...
	const handleSubmitAllFiles = async () => {
		...
		(Object.keys(FIELD_MAP) as DocumentField[]).forEach((frontendField) => {
			...
			if (file) {
				formData.append(backendField, file);
				console.log(`Added ${backendField}:`, file.name);
			}
		});
		...
		try {
			...
			console.log("Submitting documents to:", URL);
			...
			if (responseData.driver?.id) {
				...
				console.warn('No driver.id in response:', responseData);
			}

			console.log("Documents uploaded successfully, navigating to payment-details");
	...
	// Show error state with redirect countdown
	if (error && error.includes('Please log in again')) {
		return (
			<div className='flex items-center justify-center min-h-screen bg-gray-100'>
	...
	return (
		<div className='flex flex-col lg:flex-row items-center justify-center gap-8 p-6 bg-gray-100 min-h-screen'>
			...
			<div className='lg:w-1/2 w-full max-w-md bg-white p-6 rounded-lg shadow-md'>
				...
				{error && !error.includes('Please log in again') && (
					<div className='p-4 mb-4 bg-red-100 text-red-700 rounded-md border border-red-400'>
						<p className='font-medium'>Error</p>
						<p className='text-sm'>{error}</p>
					</div>
				...
				<button
					...
					className={`w-full py-3 px-4 rounded-md font-medium text-white transition-all duration-300
            ${
							isSubmitting || !user_id || !authToken
								? 'bg-gray-400 cursor-not-allowed'
								: Object.values(uploadedFiles).every((file) => file !== null)
								? 'bg-blue-600 hover:bg-blue-700 active:bg-blue-800'
								: 'bg-gray-400 cursor-not-allowed'
				...
				{process.env.NODE_ENV === 'development' && (
					<div className='mt-4 p-3 bg-gray-100 rounded text-xs'>
						...
						<p>Token: {authToken ? 'Available' : 'Not found'}</p>
						<p>Files: {Object.values(uploadedFiles).filter(f => f).length}/4</p>
					</div>
				)}
			</div>
		</div>
